...
"[project]/app/api/movies/[slug]/route.ts [app-route] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
(()=>{
    const e = new Error("Cannot find module '@/lib/tmdb'");
    e.code = 'MODULE_NOT_FOUND';
    throw e;
})();
;
...
async function GET(_, { params }) {
    ...
    let { data: movie } = await db.from('movies').select('*').eq('slug', slug).maybeSingle();
    if (!movie) {
        ...
        const ins = await db.from('movies').upsert(row, {
            onConflict: 'tmdb_id'
        }).select().single();
        movie = ins.data;
    }
    const { data: v } = await db.from('ratings_summary').select('*').eq('movie_id', movie.id).maybeSingle();
...
//# sourceMappingURL=%5Broot-of-the-server%5D__1anepx8._.js.map